...
from config import (
    CAMERA_INDEX, FRAME_WIDTH, FRAME_HEIGHT, MATCH_THRESHOLD,
    COLOR_RECOGNIZED, COLOR_STRANGER, COLOR_WELCOME,
    ENABLE_CAMERA, ALLOW_REPEAT_CHECKIN,
)
from attendance_db import AttendanceDB
from face_db import FaceDatabase
from face_recognition import CheckInTracker, FaceRecognizer
...
def pil_draw_text(draw: ImageDraw.ImageDraw, pos: tuple[int, int],
                  text: str, font_size: int, color: tuple[int, int, int],
                  stroke: bool = False) -> tuple[int, int]:
    font = _get_font(font_size)
    bbox = draw.textbbox(pos, text, font=font)
    tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
    if stroke:
        draw.text(pos, text, font=font, fill=color,
                  stroke_width=1, stroke_fill=(15, 15, 15))
    else:
        draw.text(pos, text, font=font, fill=color)
    return tw, th
...
def draw_rounded_rectangle(img, pt1, pt2, color, thickness=-1, radius=10):
    ...
    if thickness < 0: # 实心
        ...
        cv2.line(img, (x1 + radius, y1), (x2 - radius, y1), color, thickness, lineType=cv2.LINE_AA)
        cv2.line(img, (x1 + radius, y2), (x2 - radius, y2), color, thickness, lineType=cv2.LINE_AA)
        cv2.line(img, (x1, y1 + radius), (x1, y2 - radius), color, thickness, lineType=cv2.LINE_AA)
        cv2.line(img, (x2, y1 + radius), (x2, y2 - radius), color, thickness, lineType=cv2.LINE_AA)
...
def draw_texts_on_frame(frame: np.ndarray, draw_calls: list[dict]) -> np.ndarray:
    ...
    for dc in draw_calls:
        pil_draw_text(draw, dc["pos"], dc["text"], dc["size"], dc["color"], dc.get("stroke", False))
    return cv2.cvtColor(np.array(pil), cv2.COLOR_RGB2BGR)


# ──────────────────── 主流程 ────────────────────

def _run_headless_loop(cap, face_db, db_embeddings, recognizer, attendance_db, checkin_tracker):
    """无 GUI 模式：摄像头静默运行，控制台输出打卡事件，Ctrl+C 退出"""
    logger.info("无界面模式启动 (ENABLE_CAMERA=False)，摄像头静默运行中...")
        ...
        while True:
            ...
            for face in faces:
                ...
                if ALLOW_REPEAT_CHECKIN or recognizer.should_welcome(name):
                    ...
                    play_welcome()
                    logger.info("签到成功: %s (sim=%.3f, row=%s)", name, similarity, row_id)

            fps_frames += 1
            if time.time() - fps_start >= 10.0:
                fps_display = f"{fps_frames / 10:.1f}"
                logger.debug("FPS: %s, 今日已签: %d 人", fps_display,
...
def _run_gui_loop(cap, face_db, db_embeddings, recognizer, attendance_db, checkin_tracker):
        ...
        while True:
            ...
            for face in faces:
                ...
                if recognized:
                    ...
                    if not ALLOW_REPEAT_CHECKIN and checkin_tracker.is_checked_out_today(name):
                        label = f"已签退 · {name}"
                    elif not ALLOW_REPEAT_CHECKIN and checkin_tracker.is_checked_in_today(name):
                        label = f"已签到 · {name}"
                    else:
                        label = name
                        if ALLOW_REPEAT_CHECKIN or recognizer.should_welcome(name):
                ...
                draw_rounded_rectangle(frame, (lbl_x1, lbl_y1), (lbl_x2, lbl_y2), color, thickness=-1, radius=6)
                text_draws.append({
                    "text": label,
                    "pos": (lbl_x1 + pad_x, lbl_y1 + pad_y - 2),
                    "size": 16, "color": (255, 255, 255), "stroke": False
                })

            # ── 3. 逻辑处理: 下班打卡按钮事件 ──
            if button_clicked:
                button_clicked = False
                if last_recognized_name and checkin_tracker.is_checked_out_today(last_recognized_name):
                    toast_text = f"{last_recognized_name} 今日已签退，无需重复操作"
                    toast_until = time.time() + 2.5
                elif last_recognized_name and checkin_tracker.is_checked_in_today(last_recognized_name):
                    duration = attendance_db.check_out(last_recognized_name)
                    checkin_tracker.reset_checkin(last_recognized_name)
                    toast_text = f"【签退】{last_recognized_name} 成功，今日工作 {duration} 分钟"
            ...
            if now < toast_until and toast_text:
                ...
                draw_rounded_rectangle(toast_overlay, (tx - 20, ty - 12), (tx + tw + 20, ty + th + 12), (20, 20, 20), -1, radius=8)
                cv2.addWeighted(toast_overlay, 0.8, frame, 0.2, 0, frame)
                draw_rounded_rectangle(frame, (tx - 20, ty - 12), (tx + tw + 20, ty + th + 12), (100, 240, 255), 1, radius=8)
                text_draws.append({"text": toast_text, "pos": (tx, ty), "size": 18, "color": (100, 240, 255), "stroke": False})
            ...
            if now < welcome_until:
                ...
                text_draws.append({
                    ...
                    "size": 22, "color": (255, 255, 255), "stroke": False
                })
            ...
            text_draws.append({
                ...
                "size": 18, "color": (22, 22, 22), "stroke": False
            })

            # ── 7. 顶部状态栏文字 ──
            text_draws.append({"text": "SYSTEM DISPATCH", "pos": (15, 8), "size": 11, "color": (160, 160, 160)})
            text_draws.append({"text": f"底库: {len(db_embeddings)} 人  |  今日已签: {sum(1 for v in checkin_tracker._records.values() for _ in v)} 人",
                               "pos": (15, 24), "size": 16, "color": (245, 245, 245)})
            ...
            if time.time() - fps_start >= 1.0:
                ...
                fps_frames = 0
                fps_start = time.time()
            text_draws.append({"text": fps_display, "pos": (w - 75, 20), "size": 14, "color": COLOR_CYAN})
...
def main():
    ...
    if ENABLE_CAMERA:
        _run_gui_loop(cap, face_db, db_embeddings, recognizer, attendance_db, checkin_tracker)
    else:
        _run_headless_loop(cap, face_db, db_embeddings, recognizer, attendance_db, checkin_tracker)
